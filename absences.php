...
if ($resetfilters !== '') {
    redirect(new moodle_url('/local/inventario/absences.php'));
}
...
$filterform = new local_inventario_absence_filters_form(
    new moodle_url('/local/inventario/absences.php'),
...
foreach ($absences as $absence) {
    if ($absence->timestart <= $now && $absence->timeend >= $now) {
        $currentabsences[] = $absence;
        continue;
    }
    $futureabsences[] = $absence;
}
$absences = array_merge($currentabsences, $futureabsences);
...
$PAGE->set_url(new moodle_url('/local/inventario/absences.php', array_filter([
    'userid' => $userid,
    'typeid' => $typeid,
...
$PAGE->set_title(get_string('absence_dashboard', 'local_inventario'));
$PAGE->set_heading(get_string('absence_dashboard', 'local_inventario'));
...
<div class="inventario-absence-dashboard mb-4">
    <div class="d-flex justify-content-between align-items-center mb-3 flex-wrap gap-2">
        <div>
            <h2 class="h4 mb-0"><?php echo get_string('absence_dashboard', 'local_inventario'); ?></h2>
            <p class="text-muted mb-0 small"><?php echo get_string('absence_dashboard_description', 'local_inventario'); ?></p>
        </div>
        <div class="d-flex gap-2 flex-wrap">
            ...
            <?php endif; ?>
            <a class="btn btn-outline-secondary" href="<?php echo (new moodle_url('/local/inventario/absences_archive.php'))->out(); ?>">
                <?php echo get_string('absence_archive', 'local_inventario'); ?>
            </a>
            <?php if ($canconfigure): ?>
                ...
                <?php if (empty($absences)): ?>
                    <tr>
                        <td colspan="7" class="text-center text-muted py-5">
                            <?php echo get_string('noabsencesyet', 'local_inventario'); ?>
                        </td>
                    </tr>
                <?php else: ?>
                    <?php foreach ($absences as $absence): ?>
                        ...
                        $current = $absence->timestart <= time() && $absence->timeend >= time();
                        $rowclasses = $current ? 'table-success' : '';
                        $allowactions = $service->can_manage_entry($absence, $USER->id);
                        ?>
                        <tr class="<?php echo $rowclasses; ?>">
                            <td><?php echo s($teachername); ?></td>
                            <td><?php echo s($period); ?></td>
                            <td>
                                <?php echo s($absence->subject); ?>
                                <?php if ($current): ?>
                                    <span class="badge bg-success ms-2 align-middle">
                                        <?php echo get_string('absence_active', 'local_inventario'); ?>
                                    </span>
                                <?php endif; ?>
                            </td>
                            <td>
                            ...
                            <td>
                                <?php if ($allowactions): ?>
                                    ...
                                    $editicon = $OUTPUT->pix_icon('t/edit', $editlabel);
                                    $deleteicon = $OUTPUT->pix_icon('t/delete', $deletelabel);
                                    ?>
                                    <div class="inventario-object-actions inventario-absence-actions">
                                        <a href="<?php echo (new moodle_url('/local/inventario/absence.php', ['id' => $absence->id]))->out(); ?>"
                                           class="inventario-object-action"
                                           title="<?php echo s($editlabel); ?>"
                                           aria-label="<?php echo s($editlabel); ?>">
                                            <?php echo $editicon; ?>
                                        </a>
                                        <form method="post" action="<?php echo (new moodle_url('/local/inventario/absence.php'))->out(); ?>" class="d-inline mb-0">
                                            ...
                                            <button type="submit"
                                                    ...
                                                    onclick="return confirm('<?php echo get_string('confirmdeleteabsence', 'local_inventario'); ?>');">
                                                <?php echo $deleteicon; ?>
                                            </button>
                                        </form>
                                    </div>
